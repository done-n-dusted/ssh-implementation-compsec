		     ...
		     unsigned char *buffer, unsigned int *len )
{
	...
	*/
	...
	unsigned char ivlen = plaintext_len;
	unsigned char *iv;
	...
	// buffer = tag + ivlen + clen + IV + ciphertext
...
int seal_symmetric_key( unsigned char *key, unsigned int keylen, EVP_PKEY *pubkey, char *buffer )
{
	...
	*/
	...
	len = rsa.encrypt(key, keylen, &ciphertext, &ek, &ekl, &iv, &ivl, pubkey);
	...
	// buffer = buffer - len - ekl - sizeof(unsigned int) - sizeof(unsigned int);
	return len;
}
...
int unseal_symmetric_key( char *buffer, unsigned int len, EVP_PKEY *privkey, unsigned char **key )
{
	...
	*/
	...
	unsigned char *ek, *iv, *ciphertext;
...
int client_authenticate( int sock, unsigned char **session_key )
{
	...
	*/
	...
	unsigned char *pub_key = malloc(MAX_BLOCK_SIZE);
	wait_message(sock, &initResponse, &pub_key, SERVER_INIT_RESPONSE);

	// create symmetric key 

	unsigned char *symmetric_key;
	unsigned char *buffer;
	int sym_out = generate_pseudorandom_bytes(symmetric_key, MAX_BLOCK_SIZE);

	int len = seal_symmetric_key(symmetric_key, MAX_BLOCK_SIZE, pub_key, buffer);
	...
	*/
	...
	send_message(sock, &initAck, pub_key);
	...
	*/
	...
	wait_message(sock, &serverInitAck, &message, SERVER_INIT_RESPONSE);
	printf("Received message from server %s\n", message);
	int len = serverInitAck.length;

	int unseal_out = unseal_symmetric_key(buffer, len, pub_key, symmetric_key);

	/*
	* Store the Symmetric key in session_key for later use. 
	*/
	memcpy(session_key, symmetric_key, len);
